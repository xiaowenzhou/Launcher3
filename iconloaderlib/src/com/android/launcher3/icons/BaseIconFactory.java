    ...
    private Drawable normalizeAndWrapToAdaptiveIcon(@NonNull Drawable icon,
        ...
        if (shrinkNonAdaptiveIcons && ATLEAST_OREO) {
            ...
            if (!(icon instanceof AdaptiveIconDrawable) && !outShape[0]) {
				...
				((ColorDrawable) dr.getBackground()).setColor(DEFAULT_WRAPPER_BACKGROUND);
            }
        } else {
            scale = getNormalizer().getScale(icon, outIconBounds, null, null);
        }
         
		icon= RoundedBitmapDrawableFactory.create(mContext.getResources(),drawableToBitmap(icon,scale));
        ((RoundedBitmapDrawable) icon).setCornerRadius(18.0f);
        outScale[0] = scale;
        return icon;
    }
	
	private Bitmap drawableToBitmap(Drawable drawable,float scale) {
